...
\begin{equation}
    R_t = F_t + \epsilon_t,
\end{equation}
where $E[F_t] = 0$ and $E[\epsilon_t] = 0, \text{cov}(\epsilon_t, \epsilon_{t-k}) = \sigma^2 \delta_k$.
...
We compute the expectations of several quantities over $T$ steps (as $T \to \infty$).

\subsection{Ideal P\&L (IPNL)}
The expected ideal P\&L is:
\begin{equation}
    E[\text{IPNL}] = E\left[ \sum_{t=1}^T P_t F_t \right] = A T \sum_{k=0}^\infty \lambda^k E[F_t F_{t-k}] = A T \phi(\lambda) = A T \frac{\rho(0)}{1 - \lambda \nu}.
\end{equation}
...
\begin{equation}
    E[\text{SSI}] = \mu \frac{\sigma}{\text{ADV}} E\left[ \sum (P_t - P_{t-1})^2 \right]
\end{equation}
...
\begin{equation}
    E[\text{SSR}] = \gamma \sigma^2 E\left[ \sum P_t^2 \right] = A^2 T \gamma \sigma^2 E[G_t^2].
\end{equation}
For AR(1) forecasts:
\begin{equation}
